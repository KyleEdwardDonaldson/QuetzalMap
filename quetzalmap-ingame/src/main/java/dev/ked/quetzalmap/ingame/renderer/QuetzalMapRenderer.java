...
public class QuetzalMapRenderer extends MapRenderer {
    ...
    public QuetzalMapRenderer(QuetzalMapPlugin plugin, Player player) {
        super(true); // contextual - so canvas persists between renders
        this.plugin = plugin;
        this.player = player;
    }
    ...
    public void render(@NotNull MapView map, @NotNull MapCanvas canvas, @NotNull Player player) {
        // With contextual rendering, the canvas is cached
        // We only need to render once or when things change

        // For overlays like storms and regions, we need to redraw them
        // But we should do it sparingly to avoid lag

        // Check if we need to update (every 20 ticks / 1 second)
        if (!initialized || System.currentTimeMillis() % 1000 < 50) {
            initialized = true;

            // Render overlays (storms, regions) on top of terrain
